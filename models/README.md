...
当然多segment是有代价的，就是原bert的segment需要被弃用，需要在代码的`def load_weights_from_checkpoint`（Transformer类的类方法）中将`Embedding-Segment`移除`mapping`（`mapping.pop('Embedding-Segment'）`)从而不再加载这一部分的预训练权重。
